const site = [
  ...
  },
  {
    ...
    description:
      "rorec collagen hyaluronic acid white face serum riceNet weight: 15 m",
    price: 46,
...
site.forEach((member) => {
  ...
  if (member.id < 10) {
    ...
    div.id = member.id
    div.innerHTML = ` <div class="card item" > <div class="card-header" ><img src="${member.thumbnail}" class="image" alt=""></img>  </div>  <div class="card-body"> <h5 class="card-title">${member.title}</h5><p class="card-text">Description: ${member.description}</p><p class="card-text">Price: ${member.price}</p><p class="card-text">Discount: ${member.discountPercentage}</p><p class="card-text">Rating: ${member.rating}</p> <p class="card-text">Stock: ${member.stock}</p><p class="card-text">Brand: ${member.brand}</p><p class="card-text">Category: ${member.category}</p> <a href="#" class="btn btn-dark">Buy Now</a> </div></div></div></div>`;
...
const searchFn = (e) => {
  if (e==="") return;
  site.forEach((member) => {
    var card = document.getElementById(member.id);
    if (!member.title.toLowerCase().includes(e.toLowerCase())) {
